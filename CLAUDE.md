...
After making any code changes, run the relevant tests or start the dev server to check for errors before responding. Never say "done" if the code is untested.In general do not move onto your next todo unless you are 95% confident in your work.For each major step in terms of front design:
1. Take a screenshot and verify the layout looks correct(use this skill : https://github.com/Leonxlnx/taste-skill)
2. Open Chrome dev tools and check for console errors and fix them before responding.
3. Only move on once everything passes
